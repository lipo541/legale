...
// Hooks
import { useCompanySpecialists } from './hooks/useCompanySpecialists'
...
// ============================================================================
// Main Component - Company Specialists Page
// ============================================================================

export default function CompanySpecialistsPage() {
  ...
  const [editingSpecialist, setEditingSpecialist] = useState<CompanySpecialistProfile | null>(null)
  ...
  const [changeCompanyDropdownId, setChangeCompanyDropdownId] = useState<string | null>(null)
  ...
  const handleStartEdit = useCallback(async (specialist: CompanySpecialistProfile) => {
  ...
  // -------------------------------------------------------------------------
  const renderExpandedContent = useCallback((specialist: CompanySpecialistProfile) => {
  ...
  return (
    <div className={`min-h-full px-4 sm:px-6 lg:px-8 py-4 ${isDark ? 'text-white' : 'text-black'}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className={`text-[10px] flex items-center gap-1.5 ${isDark ? 'text-white/50' : 'text-black/50'}`}>
                ...
                <span>კომპანიების სპეციალისტების მართვა</span>
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="mb-4 grid grid-cols-2 gap-2 md:grid-cols-6">
        <StatsCard label="სულ" value={stats.total} isDark={isDark} icon={Users} />
      ...
      <BulkActionsBar
        ...
        onBulkDelete={() => handleBulkDelete(Array.from(selectedIds), clearSelection)}
        onBulkBlock={() => handleBulkBlock(Array.from(selectedIds), true, clearSelection)}
        onBulkUnblock={() => handleBulkBlock(Array.from(selectedIds), false, clearSelection)}
      ...
      {loading.fetching ? (
        ...
        <div className={`rounded-xl border p-8 text-center ${
          ...
          <Building2 className={`mx-auto mb-2 h-8 w-8 ${isDark ? 'text-white/20' : 'text-black/20'}`} />
          <p className={`text-xs ${isDark ? 'text-white/60' : 'text-black/60'}`}>
            {filters.searchTerm || filters.verificationFilter !== 'ALL' || filters.blockFilter !== 'ALL' || filters.companyFilter
              ? 'სპეციალისტები ვერ მოიძებნა' 
              : 'კომპანიის სპეციალისტები ჯერ არ არის'}
          </p>
        </div>
      ) : (
        <>
          <SpecialistTable
            ...
            changeCompanyDropdownId={changeCompanyDropdownId}
            selectedCompanyId={selectedCompanyId}
            onToggleChangeCompanyDropdown={setChangeCompanyDropdownId}
