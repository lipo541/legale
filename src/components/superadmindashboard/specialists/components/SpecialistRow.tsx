...
// ============================================================================
// Specialist Row - Memoized Component (for Company Specialists)
// ============================================================================
...
interface SpecialistRowProps {
  ...
  changeCompanyDropdownId: string | null
  onToggleChangeCompanyDropdown: (id: string | null) => void
...
const SpecialistRow = memo(function SpecialistRow({
  ...
  onChangeCompany,
  onSelectCompany,
  changeCompanyDropdownId,
  onToggleChangeCompanyDropdown,
  children,
  translationsContent,
  isMobileView = false
}: SpecialistRowProps) {
  const isChangingCompany = changeCompanyDropdownId === specialist.id
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-1.5 flex items-center gap-1.5">
              ...
              <span className={`text-[10px] truncate ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
            ...
            <div className="mt-2.5 flex items-center gap-1 flex-wrap">
              ...
              <div className="relative">
                <button
                  onClick={() => onToggleChangeCompanyDropdown(isChangingCompany ? null : specialist.id)}
                ...
                {/* Change Company Dropdown - Mobile */}
                {isChangingCompany && (
                  <div className={`absolute left-0 top-full mt-1 w-64 rounded-lg border shadow-lg z-50 ${
                    ...
                    <div className="p-2 space-y-2">
                      ...
                      >
                        ...
                        {companies.filter(c => c.id !== specialist.company_id).map((company) => (
                      ...
                      <div className="flex gap-1.5">
                        <button
                          onClick={() => {
                            ...
                            if (company) {
                              onChangeCompany(specialist.id, selectedCompanyId, company.full_name)
                            }
                          }}
                        ...
                        <button
                          onClick={() => {
                            onToggleChangeCompanyDropdown(null)
                            onSelectCompany('')
                          }}
                          className={`flex-1 rounded-md px-2 py-1.5 text-xs font-medium transition-colors ${
                            isDark
              ...
              {/* Convert to Solo */}
              <button
                onClick={() => onConvertToSolo(specialist)}
                disabled={loading.convertingToSolo === specialist.id}
                className={`rounded-md p-1.5 transition-colors ${
                  isDark ? 'bg-white/10' : 'bg-black/5'
                }`}
                title="სოლოდ გადაყვანა"
              >
                {loading.convertingToSolo === specialist.id ? (
                  <Loader2 className="h-4 w-4 animate-spin text-purple-500" />
                ) : (
                  <UserMinus className={`h-4 w-4 ${isDark ? 'text-purple-400' : 'text-purple-600'}`} />
                )}
              </button>
  ...
  return (
    <Fragment>
      <tr className={`border-t transition-colors ${isDark ? 'border-white/10 hover:bg-white/5' : 'border-black/10 hover:bg-black/5'}`}>
        ...
        <td className="px-2 py-2">
          <div className="flex items-center gap-1.5">
            <Building2 className={`h-3 w-3 flex-shrink-0 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} />
            <span className={`text-[10px] truncate max-w-[120px] ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
              {specialist.company_name || 'N/A'}
            </span>
          </div>
        </td>
        ...
        <td className="px-2 py-2">
          <div className="flex items-center justify-end gap-1">
            ...
            <button
              ...
              title="სოლო სპეც.-ად გადაყვანა"
            >
              {loading.convertingToSolo === specialist.id ? (
                <Loader2 className="h-3.5 w-3.5 animate-spin text-purple-500" />
              ) : (
                <UserMinus className={`h-3.5 w-3.5 ${isDark ? 'text-purple-400' : 'text-purple-600'}`} />
              )}
            </button>
