...
export default function PracticePageClient({ initialData, locale }: PracticePageClientProps) {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>(
    ...
    (typeof window !== 'undefined' ? (localStorage.getItem('practices-view-mode') as ViewMode) : null) || 
    'grid'
  )
        ...
        <div className="mb-8 space-y-4">
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <Sort options={sortOptions} value={sortBy} onChange={(value) => setSortBy(value as SortOption)} />
            </div>
            <ViewModeToggle view={viewMode} onChange={(value) => setViewMode(value as ViewMode)} />
          </div>
        </div>
