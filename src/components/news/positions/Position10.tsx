...
interface Position10Props {
  posts: Post[]
}

// Featured Post Card - Same as Position 6
export default function Position10({ posts }: Position10Props) {
  ...
  if (!post) {
    return (
      <div className={`flex h-full flex-col items-center justify-center rounded-xl border border-dashed p-4 ${
        isDark ? 'border-white/20 bg-white/5' : 'border-gray-700 bg-black'
      }`}>
        <BookOpen className={`mb-2 h-6 w-6 ${isDark ? 'text-white/30' : 'text-white/40'}`} />
        <p className={`text-xs text-center ${isDark ? 'text-white/40' : 'text-white/60'}`}>
          {t.noPostsPosition}
        </p>
      </div>
    )
  ...
  if (!translation) {
    return (
      <div className={`flex h-full items-center justify-center rounded-xl ${
        ...
        <p className={`text-xs ${isDark ? 'text-white/40' : 'text-white/60'}`}>{t.translationNotFound}</p>
      </div>
    )
  }
